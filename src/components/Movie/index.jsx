...
export const Movie = (props) => {
  ...
  React.useEffect(() => {
    if (pinnedItems && pinnedItems.some(item => item.kinopoiskId === props.kinopoiskId)) {
      setIsPinned(true);
    }
  }, [pinnedItems, props.kinopoiskId]);

  const onClickDetails = (id) => {
    navigate(`/film/${id}`);
  }

  const onClickPinn = (id) => {
  ...
  return (
    <div className={`movie ${isPinned ? 'pinned' : ''}`}>
      ...
      <div className="movie__desc">
        <span className="movie__desc-year">{props.year}</span>
        <svg
          width="3"
          height="3"
          viewBox="0 0 3 3"
        ...
        <span className="movie__desc-genre">
          {props.genres.length > 0 ? props.genres[0].genre : ""}
        </span>
      </div>
      <h2 className="movie__name" onClick={() => onClickDetails(props.kinopoiskId)}>{props.nameOriginal}</h2>
      <div className="movie__pinn" onClick={() => onClickPinn(props.kinopoiskId)}>
        <svg
          width="12"
          height="14"
          viewBox="0 0 12 14"
